runDimReduc=function(object,DimReducMethod = "PCA",
                assay = "SCT",
                percentCut = 0.05,
                pcaDim=30,...) {
  ...
  if(DimReducMethod == "PCA") {
    ...
    ##Can't use CreateSpatialPCAObject here as it will use SCTransform for normlization ,which is not good for image features
    ##Use @scale.data directly
    SPCAobj <- new(Class = "SpatialPCA", counts = rawcountFiltered, location = locationFiltered,
