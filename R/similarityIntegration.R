...
IntegrationByDistance=function(dataObj=NULL,
                               distMatrix=c("SCTBayesSpace_dist_Cluster10","ImageFeatureBayesSpace_dist_Cluster10"),
                               snnMatrix=c("SCTBayesSpace_snn_Cluster10","ImageFeatureBayesSpace_snn_Cluster10"),
                               name="DistIntegrated",
                               ...
                               k.param=20,minDiff=10^-4) {

  if (is.null(dataObj)) { #not defined dataObj, distMatrix should be a list of distance Matrixs
  ...
  #head(data.frame(distMatrix1InNeighbor1,distMatrix1InNeighbor2,distMatrix2InNeighbor1,distMatrix2InNeighbor2))
  snnMatrix1SmallIndexList=list()
  for (i in 1:nrow(snnMatrix1)) {
    snnMatrix1SmallIndexList[[i]]=order(snnMatrix1[i, ])[1:k.param] #subject Ind with smallest non zero SNN (Jarcard index)
  }
  snnMatrix2SmallIndexList=list()
  for (i in 1:nrow(snnMatrix2)) {
    snnMatrix2SmallIndexList[[i]]=order(snnMatrix2[i, ])[1:k.param] #subject Ind with smallest non zero SNN (Jarcard index)
  }
  #browser()

  distMatrix1InSmallIndex1=NeighborDist(distMatrix1,snnMatrix1SmallIndexList,removeFirst=FALSE) #distance to farest neighbors. don't need to remove first (self)
  distMatrix2InSmallIndex2=NeighborDist(distMatrix2,snnMatrix2SmallIndexList,removeFirst=FALSE)

  #caculate values
  theta1InNeighbor1=exp(-pmax((distMatrix1InNeighbor1[,1]-distMatrix1InNeighbor1[,2]),0)/(distMatrix1InSmallIndex1[,1]-distMatrix1InNeighbor1[,2]+minDiff))
  theta1InNeighbor2=exp(-pmax((distMatrix1InNeighbor2[,1]-distMatrix1InNeighbor1[,2]),0)/(distMatrix1InSmallIndex1[,1]-distMatrix1InNeighbor1[,2]+minDiff))

  theta2InNeighbor1=exp(-pmax((distMatrix2InNeighbor1[,1]-distMatrix2InNeighbor2[,2]),0)/(distMatrix2InSmallIndex2[,1]-distMatrix2InNeighbor2[,2]+minDiff))
  theta2InNeighbor2=exp(-pmax((distMatrix2InNeighbor2[,1]-distMatrix2InNeighbor2[,2]),0)/(distMatrix2InSmallIndex2[,1]-distMatrix2InNeighbor2[,2]+minDiff))
...
FindNeighborsByDistance=function(dataObj=NULL,distMatrix,prune.SNN=1/15,
                                 assay=NULL,
