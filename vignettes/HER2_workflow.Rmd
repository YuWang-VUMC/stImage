...
HER2 breast cancer (Spatial Transcriptomics platform) dataset can be accessed at https://doi.org/10.5281/zenodo.4751624. After downloading and unzip, the following files will be needed:

- [the gene expression matrix](H1.tsv)

- [Coordinates](H1_position.csv)

- [raw H&E image (TIFF)](HE_BT24044_D2.jpg)
...
positionTableFile <- file.path(data_path, "H1_position.csv")
positionTable <- read.csv(positionTableFile, header = F, row.names = 1)
...
imageFeatures_list <- ExtractFeaturesVisium(imgFile = rawimg,
...
RGBquantile <- read.table(system.file("extdata", "HER2.H1_RGBprofile.tsv", package = "stImage"), header = T)
...
```{r preprocessing, message = F, warning = F, echo = T, eval = T}
clusterNum=6
#PCA
...
```{r bayesspace, message = F, warning = F, echo = T, eval = T}
...
```{r spatialpca, message = F, warning = F, echo = T, eval = T}
...
```{r stlearn, message = F, warning = F, echo = T, eval = T}
...
```{r pca_int, message = F, warning = F, echo = T, eval = T}
...
```{r bayesspace_int, message = F, warning = F, echo = T, eval = T}
...
```{r spatialpca_int, message = F, warning = F, echo = T, eval = T}
...
```{r stlearn_int, message = F, warning = F, echo = T, eval = T}
